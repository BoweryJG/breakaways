...
class BreakawaySound {
    constructor() {
        ...
        // Enhanced sound registry system
        this.soundRegistry = new Map(); // All active sounds with metadata
    ...
    // Sound registry methods
    registerSound(soundData) {
        const id = soundData.id || `sound_${++this.soundIdCounter}`;
        const sound = {
            id: id,
            name: soundData.name || 'Unnamed Sound',
            type: soundData.type || 'unknown',
            category: soundData.category || 'uncategorized',
            frequency: soundData.frequency || null,
            source: soundData.source || 'internal',
            startTime: Date.now(),
            duration: soundData.duration || null,
            volume: soundData.volume || 1.0,
            metadata: soundData.metadata || {},
            isPlaying: true,
            oscillators: soundData.oscillators || [],
            gainNodes: soundData.gainNodes || []
        };
        
        this.soundRegistry.set(id, sound);
        this.emit('soundRegistered', sound);
        return id;
    }
    
    unregisterSound(id) {
        const sound = this.soundRegistry.get(id);
        if (sound) {
            this.soundRegistry.delete(id);
            this.emit('soundUnregistered', sound);
        }
    }
    ...
    updateSound(id, updates) {
        ...
        if (sound) {
            ...
            this.emit('soundUpdated', sound);
        }
    }
    
    getActiveSound(id) {
        return this.soundRegistry.get(id);
    }
    
    getAllActiveSounds() {
        return Array.from(this.soundRegistry.values());
    }
    
    getActiveSoundsByCategory(category) {
        return Array.from(this.soundRegistry.values())
            .filter(sound => sound.category === category);
    }
    
    getActiveSoundsBySource(source) {
        return Array.from(this.soundRegistry.values())
    ...
    playSacredFrequency(frequencyName, duration = null, fadeIn = 0.1, fadeOut = 0.1) {
        ...
        this.registerSound({
            id: id,
            name: `Sacred Frequency: ${frequencyName}`,
            type: 'sacred',
            category: 'sacred',
            frequency: frequency,
            source: 'sound-system',
            ...
            oscillators: [oscillator, harmonic2, harmonic3],
            gainNodes: [gainNode]
        });
    ...
    stopSound(id) {
        ...
        // Emit stop event
        const sound = this.getActiveSound(id);
        if (sound) {
            this.emit('soundStopped', sound);
        }
    }
